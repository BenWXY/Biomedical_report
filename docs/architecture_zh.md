...
3. `IntelligencePipeline.from_settings()` 创建缓存、HTTP 客户端、数据源列表、可选 LLM 分析器和报告渲染器。离线模式下不会创建 LLM 分析器。
4. 每个 `DataSource` 根据靶点抓取数据，并返回 `SourceResult`。
5. 管线将记录拆分为 `TrialRecord` 和 `PublicationRecord`，去重后生成 `IntelligenceBundle`。
6. 在实时模式中，`LLMReportAnalyzer` 可选地通过 OpenAI Python SDK 调用 OpenAI-compatible Chat Completions API。
...
离线模式是完全本地的：不会调用公开 API，也不会调用 LLM，即使已经配置了 LLM 凭据。
...
LLM 层是可选的，只影响报告正文分析，不负责数据采集，并且在离线模式下会被禁用。
